...
def get_mbn_m3u8(driver):
    ...
    try:
        ...
        if auth_urls:
            ...
            # 从认证链接获取真实m3u8地址
            real_url = get_real_mbn_url(auth_urls[0])
            if real_url:
                return real_url
        
        # 如果认证链接方法失败，尝试直接访问已知地址
        print("🔄 尝试直接访问MBN地址...")
        try:
            test_urls = [
            ...
            for test_url in test_urls:
                try:
                    ...
                    if response.status_code == 200:
                        print(f"✅ 直接访问成功: {test_url}")
                        return test_url
                except:
                    continue
        except Exception as e:
            print(f"⚠️ 直接访问测试失败: {e}")
